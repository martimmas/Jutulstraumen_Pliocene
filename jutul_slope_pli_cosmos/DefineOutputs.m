...
% CtrlVar.DefineOutputs='-log10(C)-log10(AGlen)-save-';
CtrlVar.DefineOutputs='-log10(SurfSpeed)-s-save-';
...
if contains(plots,'-save-')
   ...
   if strcmp(CtrlVar.DefineOutputsInfostring,'First call ') && exist(fullfile(cd,[UserVar.IOFilesPath,'/ResultsFiles']),'dir')~=7 %;
        mkdir([UserVar.IOFilesPath,'/outputs']) ;
    end
    ...
    if strcmp(CtrlVar.DefineOutputsInfostring,'Last call')==0
        ...
        FileName=sprintf('%s/outputs/%s-%s-%07i',UserVar.IOFilesPath,UserVar.OutputExpName,UserVar.RunType,round(100*CtrlVar.time));
        fprintf(' Saving data in %s \n',FileName)
        save(FileName,'CtrlVar','MUA','s','b','S','B','h','ub','vb','ud','vd','C','dhdt','AGlen',...
                      'm','n','rho','rhow','as','ab','GF','GLgeo','xGL','yGL')
...
if contains(plots,'-B-')
    ...
    PlotMuaBoundary(CtrlVar,MUA,'b')
    title(sprintf('bedrock at t=%-g ',CtrlVar.time)) ; xlabel('x (km)') ; ylabel('y (km)')
end
...
if contains(plots,'-s-')        
    ...
    [xGL,yGL,GLgeo]=PlotGroundingLines(CtrlVar,MUA,GF,GLgeo,xGL,yGL,'g');
    PlotMuaBoundary(CtrlVar,MUA,'b')
    title(sprintf('ice surface at t=%-g ',CtrlVar.time)) ; xlabel('x (km)') ; ylabel('y (km)')
end
