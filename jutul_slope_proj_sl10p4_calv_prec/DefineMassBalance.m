...
if contains(UserVar.RunType,'Forward') ==1 % i.e., diagnostic and transient
    ...
    switch UserVar.Qbm.Mode
        ...
        case 'ReadFromFile'
            ab=-1.*Fab(MUA.coordinates(:,1),-1.*MUA.coordinates(:,2));
            ab(abs(ab)>1e10) = 0;        
        ...
        case 'PICO'
            ...
            if isfield(UserVar.Qbm,'GLenhance') && UserVar.Qbm.GLenhance==1            
...
% Final modification: increase in accumulation (7% increase per deg. C;
% 5 deg. C warming based on SSP5-8.5 at 2090s (Tebaldi et al., 2021; ESD, doi: https://doi.org/10.5194/esd-12-253-2021)
as(as>0) = as(as>0).*(1. + 0.07*5);
